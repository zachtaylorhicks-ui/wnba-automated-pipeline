...
def get_current_season_data(current_year: int) -> Optional[pd.DataFrame]:
    ...
    season_types = ['Regular Season', 'Playoffs']; refreshed_data = []
...
def calculate_advanced_stats(df):
    ...
    team_game_stats = df.groupby(['gameId', 'playerteamName']).agg(FGA=('fieldGoalsAttempted', 'sum'), FTA=('freeThrowsAttempted', 'sum'), OREB=('reboundsOffensive', 'sum'), TOV=('turnovers', 'sum')).reset_index()
    ...
    team_game_stats = pd.merge(team_game_stats, game_dates, on='gameId'); team_game_stats.sort_values(by=['playerteamName', 'gameDate'], inplace=True)
    for stat in ['Poss', 'OffRtg']: team_game_stats[f'roll_{stat}'] = team_game_stats.groupby('playerteamName')[stat].transform(lambda x: x.rolling(window=10, min_periods=1).mean().shift(1))
    team_game_stats.fillna(0, inplace=True); df = pd.merge(df, team_game_stats[['gameId', 'playerteamName', 'roll_Poss', 'roll_OffRtg']], on=['gameId', 'playerteamName'], how='left')
    ...
    df = pd.merge(df, opp_stats, on=['gameId', 'playerteamName'], how='left'); df.fillna(0, inplace=True); logging.info("Advanced stats calculation complete."); return df

def engineer_player_features(df):
    logging.info("Engineering player-level features...")
    df.sort_values(by=['personId', 'gameDate'], inplace=True); stats_to_average = ['points', 'assists', 'reboundsTotal', 'steals', 'blocks', 'turnovers', 'fieldGoalsAttempted', 'threePointersAttempted']; windows = [3, 5, 10]
    for col in stats_to_average:
        for w in windows: df[f"{col}_roll_{w}"] = df.groupby('personId')[col].transform(lambda x: x.rolling(window=w, min_periods=1).mean().shift(1))
    df['days_rest'] = df.groupby('personId')['gameDate'].diff().dt.days; df[['days_rest'] + [f"{s}_roll_{w}" for s in stats_to_average for w in windows]] = df[['days_rest'] + [f"{s}_roll_{w}" for s in stats_to_average for w in windows]].fillna(0); df.loc[df['days_rest'] > 14, 'days_rest'] = 14; return df

def run_wiki_enrichment(base_data_path, cache_path):
    WNBA_ALL_PLAYERS_URL = "https://en.wikipedia.org/wiki/List_of_Women%27s_National_Basketball_Association_players"; PLAYER_ID_NAME_MAP, NORMALIZED_NAME_TO_ID_MAP, NORMALIZED_CANONICAL_NAMES = {}, {}, []
    def normalize_name(name):
        if not isinstance(name, str): return ""
        name = "".join(c for c in unicodedata.normalize('NFKD', name.lower()) if not unicodedata.combining(c)); return name if name.strip() else ""
    def build_player_universe(hist_df):
        nonlocal PLAYER_ID_NAME_MAP, NORMALIZED_NAME_TO_ID_MAP, NORMALIZED_CANONICAL_NAMES
        # Use ORIGINAL column names here
        hist_players = hist_df[['PLAYER_ID', 'PLAYER_NAME']].dropna().drop_duplicates(subset='PLAYER_ID')
        ...
        for _, row in hist_players.iterrows():
            pid, name = int(row['PLAYER_ID']), row['PLAYER_NAME']
            PLAYER_ID_NAME_MAP[pid] = name; norm_name = normalize_name(str(name))
    ...
    # === THE FIX IS HERE ===
    # We now read the RAW column names 'PLAYER_ID' and 'PLAYER_NAME'
    hist_df = pd.read_csv(base_data_path, usecols=['PLAYER_ID', 'PLAYER_NAME'], low_memory=False)
    build_player_universe(hist_df)
    try: from fuzzywuzzy import process, fuzz
    except ImportError: logging.error("FuzzyWuzzy not found. Cannot perform wiki scraping."); return {}
    all_player_profiles = {}
    response = requests.get(WNBA_ALL_PLAYERS_URL, headers={'User-Agent': 'Mozilla/5.0'}); soup = BeautifulSoup(response.content, 'html.parser'); content_div = soup.find('div', class_='mw-parser-output')
    for link_tag in content_div.find_all('a', href=re.compile(r'^/wiki/')):
        ...
        if pid and pid not in all_player_profiles: all_player_profiles[pid] = {'personId': pid, 'playerName': PLAYER_ID_NAME_MAP.get(pid, player_name), 'wikiUrl': f"https://en.wikipedia.org{link_tag['href']}"}
    ...
    for pid, profile in all_player_profiles.items():
        ...
        try:
            time.sleep(0.01); response = requests.get(profile['wikiUrl'], headers={'User-Agent': 'Mozilla/5.0'}); response.raise_for_status(); soup = BeautifulSoup(response.content, 'html.parser'); infobox = soup.find('table', class_='infobox')
    ...
    with open(cache_path, 'w') as f: json.dump(all_player_profiles, f, indent=2); logging.info(f"Wiki cache created and saved to {cache_path}"); return all_player_profiles

def main():
    base_file = 'data/wnba_all_player_boxscores_1997-2025.csv'; wiki_cache_file = 'data/wnba_wiki_cache.json'; output_path = 'data/wnba_model_ready_data_with_features.csv'
    ...
    df = df_raw.rename(columns=rename_map); df['gameDate'] = pd.to_datetime(df['gameDate']); df['MATCHUP'] = df['MATCHUP'].astype(str); df['home'] = df['MATCHUP'].apply(lambda x: 1 if 'vs.' in x else 0)
    temp_teams = df['MATCHUP'].str.replace('vs. ', '@').str.split(' @ ', expand=True); df['opponentteamName'] = np.where(df['teamAbbreviation'] == temp_teams[0], temp_teams[1], temp_teams[0])
    df['personId'] = pd.to_numeric(df['personId'], errors='coerce'); df.dropna(subset=['personId'], inplace=True)
    df = engineer_player_features(df); df = calculate_advanced_stats(df)
    percentage_cols = ['fieldGoalsPercentage', 'threePointersPercentage', 'freeThrowsPercentage']; df[percentage_cols] = df[percentage_cols].fillna(0.0); df.dropna(subset=['numMinutes', 'points'], inplace=True); df.sort_values(by='gameDate', ascending=False, inplace=True)
    df.to_csv(output_path, index=False); logging.info(f"Success! Feature-engineered data saved to '{output_path}'")
